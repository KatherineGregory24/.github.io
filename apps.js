...
let result;
    ...
    (conversionType === "ctof") ; {
        ...
        // Assess temperature based on the result (Fahrneheit)
...
function validateDate(dateStr) {
  ...
  // Split by / and check if we have exactly 2 parts
  ...
  // Check if both parts are numbers adn have correct length
...
function validateTime(timeStr) {
  ...
  // Check if both parts are numbers and have correct length
  ...
  // Validate hour range (1-23)
  if (hourNum < 0 || hourNum > 23) {
    return false;
  }

  // Validate minute range based on hour
...
// Step 6
        ...
        try {
            const value = checkZero(display.value);
...
// This is for log(0) producing an Error inside of -infinity
function checkZero(input) {
    if (input === '0') {
        throw new Error("log(0) is undefined");
    }
    return evaluatedExpression(display.value);
}
